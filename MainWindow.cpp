...
void detectAndDisplay(VideoCapture cap);
...
vector<Rect> getGroundtruth(int frm) {
	...
	while (getline(input, currline) ){
		...
		Rect rect = Rect(atoi(line_elements[0].c_str()), atoi(line_elements[1].c_str()),
				atoi(line_elements[2].c_str()), atoi(line_elements[3].c_str()));
		res.push_back(rect);
		}
	}
	input.close();
